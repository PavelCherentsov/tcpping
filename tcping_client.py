...
class Packet:
    def __init__(self, src_ip, dest_ip, dest_port):
        ...
        self.data_offset_res_flags = (self.data_offset << 12) + (
                self.reserved << 9) + (self.ns << 8) + (self.cwr << 7) + (
                                             self.ece << 6) + (
                                             self.urg << 5) + (
                                             self.ack << 4) + (
                                             self.psh << 3) + (
    ...
    def generate_tmp_ip_header(self):
        tmp_ip_header = pack("!BBHHHBBH4s4s", self.v_ihl, self.type_of_service,
                             self.total_length,
    ...
    def generate_packet(self):
        final_ip_header = pack("!BBHHHBBH4s4s", self.v_ihl,
                               self.type_of_service, self.total_length,
                               self.identification, self.f_fo,
                               self.ttl, self.protocol, self.calc_checksum(
                self.generate_tmp_ip_header()),
                               self.src_addr,
                               self.dest_addr)
        ...
        final_tcp_header = pack("!HHLLHHHH", self.src_port, self.dest_port,
                                self.seq_no,
                                self.ack_no, self.data_offset_res_flags,
                                self.window_size,
    ...
    def send_packet(self):
        ...
        cont_send = binascii.hexlify(self.packet)
        s.settimeout(2)
        while True:
            try:
                data = s.recv(1024)
            except socket.timeout:
                print("-" + self.dest_ip + " " + str(self.dest_port))
                break
            cont_recv = binascii.hexlify(data)
            if cont_recv[40:44] == cont_send[44:48] and cont_recv[
                                                        44:48] == cont_send[
                                                                  40:44]:
                if cont_recv[66:68] == b'12':
                    print("+" + self.dest_ip + " " + str(self.dest_port))
                    break
        s.close()
        return data
...
if __name__ == '__main__':
    ...
    p = Packet(socket.gethostbyname(socket.getfqdn()), args.host, args.port)
    p.generate_packet()
